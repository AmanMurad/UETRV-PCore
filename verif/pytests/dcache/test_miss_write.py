...
async def memory_operations_test(dut):
    ...
    for idx, (addr, data, exp, sel, wr_dt) in enumerate(zip(addresses, data_values, expected, sel_byte, wr_data)):
        ...
        for i in range(4):
            if sel_bin[3 - i] == '1':  # 3-i because sel[3] is MSB byte, sel[0] is LSB byte
        ...
        if (dcache_miss==1) and (expected_result == actual_result):
            logger.info(f"Test {idx}: PASS - Actual: {hex(actual_result)}, Expected: {hex(expected_result)} with dcache_miss={dcache_miss} and write check={write_check}")
        else:
