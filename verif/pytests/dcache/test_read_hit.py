...
async def memory_operations_test(dut):
    ...
    if GEN == 1:
        ...
        addresses = [hex(generate_random_address()) for _ in range(10)]
        data_values = [hex(generate_random_data()) for _ in range(10)]
    ...
    for idx, (addr, data, exp) in enumerate(zip(addresses, data_values, expected)):
        ...
        if (actual_result_entire == expected_result_entire) and (dcache_hit == 1) and (expected_result == actual_result):
            logger.info(f"Test {idx}: PASS - Actual: {hex(actual_result)}, Expected: {hex(expected_result)} with dcache_hit={dcache_hit}")
        else:
